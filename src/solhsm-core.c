...
/*********************************GOBAL VARIABLES******************************/
static net_frame_container *container;
static zctx_t *ctx;
static zauth_t *auth;
 ...
 */
static int priv_enc(rsa_std_payload_st* rsa_rcv, void *key,
 ...
 * @return -1 if error, 0 if succeed
 */
static int priv_dec(rsa_std_payload_st* rsa_rcv, void *key,
                                uint8_t *payload, uint16_t *size);
/**
 ...
 * @return -1 if error, 0 if succeed
 */
static int pub_enc(rsa_std_payload_st* rsa_rcv, void *key,
                                uint8_t *payload, uint16_t *size);
/**
 ...
 * @return -1 if error, 0 if succeed
 */
static int pub_dec(rsa_std_payload_st* rsa_rcv, void *key,
                                uint8_t *payload, uint16_t *size);
/**
 ...
 */
static void rsa_std_error(uint8_t err_code, rsa_std_payload_st *rsa_rcv,
                                net_frame_container *c_r,
                                void*socket_zmq);
 ...
 * @return -1 if error, 0 if succeed
 */
static int sol_recv(void* socket_zmq, net_frame_container *c_r);
...
int priv_enc(rsa_std_payload_st* rsa_rcv, void *key, uint8_t *payload,
                uint16_t *size)
{
    ...
    int crypted_length = meth_rsa->rsa_priv_enc(rsa_rcv->data_length,
                                                rsa_rcv->data,
                                                cipher,
                                                priv_key ,
                                                rsa_rcv->padding);
...
int priv_dec(rsa_std_payload_st* rsa_rcv, void *key, uint8_t *payload,
                uint16_t *size)
{
    const RSA_METHOD *meth_rsa = RSA_PKCS1_SSLeay();
    int status = 0;
    ...
    int decrypted_length = meth_rsa->rsa_priv_dec(rsa_rcv->data_length,
                                                    rsa_rcv->data,
                                                    plain,
                                                    priv_key ,
                                                    rsa_rcv->padding);
...
int pub_enc(rsa_std_payload_st* rsa_rcv, void *key, uint8_t *payload,
                uint16_t *size)
{
    const RSA_METHOD *meth_rsa = RSA_PKCS1_SSLeay();
    int status = 0;
    ...
    memset(cipher, 0, sizeof(cipher));
    int crypted_length = meth_rsa->rsa_pub_enc(rsa_rcv->data_length,
                                                rsa_rcv->data,
                                                cipher,
                                                pub_key ,
                                                rsa_rcv->padding);
...
int pub_dec(rsa_std_payload_st* rsa_rcv, void *key, uint8_t *payload,
                uint16_t *size)
{
    ...
    memset(plain, 0, sizeof(plain));
    int decrypted_length = meth_rsa->rsa_pub_dec( rsa_rcv->data_length,
...
void rsa_key_error(uint8_t err_code, rsa_key_payload_st *rsa_rcv,
                    net_frame_container *c_r, void*socket_zmq)
{
    ...
    rsa_key_payload_st* rsa_snd = create_rsa_key_payload(len, rsa_rcv->key_id,
                                                RSA_KEY_ERROR,
                                                rsa_rcv->key_format,
                                                (unsigned char*)msg[id_err]);

    syslog(LOG_ERR, "Send error code: %d", err_code);
    net_frame_container* c_snd = create_container( c_r->version,
                                            c_r->payload_type,
                                            get_rsa_key_payload_size(rsa_snd));

    net_send(socket_zmq, c_snd, rsa_snd);
}

void rsa_std_error(uint8_t err_code, rsa_std_payload_st *rsa_rcv,
                    net_frame_container *c_r, void*socket_zmq)
{
    /*Each errors are pushed into syslog as LOG_ERR and we send the same message
        to the client*/
    ...
    rsa_std_payload_st* rsa_snd = create_rsa_std_payload( len, rsa_rcv->key_id,
                                                    rsa_rcv->padding,
                                                    RSA_STD_ERROR,
                                                    (unsigned char*)msg[id_err]);
    ...
    net_frame_container* c_snd = create_container( c_r->version,
                                            c_r->payload_type,
                                            get_rsa_std_payload_size(rsa_snd));

    net_send(socket_zmq, c_snd, rsa_snd);
}


int sol_recv(void* socket_zmq, net_frame_container *c_r )
{
    int status = 0;
    ...
    void* ptr_recv = net_receive(socket_zmq, c_r);
    ...
    if(c_r->payload_type == RSA_STD) {
        rsa_std_payload_st *rsa_rcv = (rsa_std_payload_st *)ptr_recv;
        if (DEBUG){
        ...
        switch (rsa_rcv->operation){
            ...
            push it to syslog */
            case RSA_PRIV_ENC:
                status = get_key_priv(rsa_rcv->key_id, get_key, key);
                if (strlen(key) == 0){
            ...
            case RSA_PUB_ENC:
                status = get_key_pub(rsa_rcv->key_id, get_key, key);
        ...
        rsa_std_payload_st* rsa_snd = create_rsa_std_payload( size,
                                                        rsa_rcv->key_id,
                                                        rsa_rcv->padding,
                                                        rsa_rcv->operation,
                                                        data);
    ...
    /*RSA KEY operations*/
        ...
        rsa_key_payload_st *rsa_rcv = (rsa_key_payload_st *)ptr_recv;
        ...
        if (rsa_rcv->key_type == RSA_KEY_PUB)
            get_key_dumm_pub(rsa_rcv->key_id, get_key, key);
        else if (rsa_rcv->key_type == RSA_KEY_DUMMY_PRIV)
             get_key_dumm_priv(rsa_rcv->key_id, get_key, key);
        else
        ...
        if (status != -1) {
            ...
            if (status != -1) {
                if(DEBUG)
                    syslog(LOG_DEBUG, "Trying to send response to the client...");
                /* Forge RSA KEY layer */
                rsa_key_payload_st* rsa_snd = create_rsa_key_payload(
...
int main()
{
    ...
    while (status != -1){
        //prepare reception
        container = malloc(sizeof(net_frame_container));
        //receive frame
